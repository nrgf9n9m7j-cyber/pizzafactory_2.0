...
# Route alternativa per compatibilità
@app.route('/api/menu/prodotti/<formato>', methods=['GET'])
def get_prodotti_menu(formato):
    """Endpoint compatibile: /api/menu/prodotti/pizza"""
    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM Prodotti WHERE Formato = ? ORDER BY Nome_Prodotto', (formato,))
